...
`urllib`를 사용하여 페이지를 읽어들이고, `BeautifulSoup`를 사용해서 앵커
...
BeautifulSoup을 사용하여 다음과 같이 각 태그별로 다양한 부분을 뽑아낼 수
있다.
...
좀더 자세한 사항은 [www.crummy.com](www.crummy.com) 에서 문서와 예제를
살펴보세요.
...
하지만, `urllib`을 사용하여, 하드 디스크 로컬 파일에 URL 사본을 쉽게
만들 수 있다.

이 패턴은 URL을 열고, `read`를 사용해서 문서 전체 내용을 다운로드하여
문자열 변수(`img`)에 다운로드하고, 그리고 나서 다음과 같이 정보를 로컬
파일에 쓴다.
...
주기억장치 `img` 변수에 저장하고, `cover.jpg` 파일을 열어 디스크에
데이터를 쓴다. 이 방식은 파일 크기가 사용자 컴퓨터의 메모리 크기보다
작다면 정상적으로 작동한다.

하지만, 오디오 혹은 비디오 파일 대용량이면, 상기 프로그램은 멈추거나
사용자 컴퓨터 메모리가 부족할 때 극단적으로 느려질 수 있다. 메모리
부족을 회피하기 위해서, 데이터를 블럭 혹은 버퍼로 가져와서, 다음 블럭을
가져오기 전에 디스크에 각각의 블럭을 쓴다. 이런 방식으로 사용자가 가진
모든 메모리를 사용하지 않고 어떠한 크기 파일도 읽어올 수 있다.
...
상기 예제에서, 한번에 100,000 문자만 읽어 오고, 웹에서 다음 100,000
문자를 가져오기 전에 `cover.jpg` 파일에 읽어온 문자를 쓴다.
...
`curl`은 "copy URL"의 단축 명령어로 두 예제는 `curl` 명령어와 비슷한
기능을 구현해서, [www.py4inf.com/code](www.py4inf.com/code) 사이트에
`curl1.py`, `curl2.py` 이름으로 올라가 있다. 동일한 작업을 좀더
효과적으로 수행하는 `curl3.py` 샘플 프로그램도 있어서 실무적으로
작성하는 프로그램에 이런한 패턴을 이용하여 구현할 수 잇다.
...
:   파이썬 라이브러리로 HTML 문서를 파싱하고 브라우저가 일반적으로
...
:   서버에 소켓 연결을 만들 때, 사용자가 무슨 응용프로그램을 연결하는지
    나타내는 숫자. 예로, 웹 트래픽은 통상 80 포트, 전자우편은 25 포트를
    사용한다.

스크래핑(scraping):

:   프로그램이 웹브라우저를 가장하여 웹페이지를 가져와서 웹 페이지의
...
:   두 응용프로그램 사이 네트워크 연결. 두 응용프로그램은 양 방향으로
    데이터를 주고 받는다.

스파이더(spider):

:   검색 색인을 구축하기 위해서 한 웹페이지를 검색하고, 그 웹페이지에
...
소켓 프로그램 `socket1.py`을 변경하여 임의 웹페이지를 읽을 수 있도록
